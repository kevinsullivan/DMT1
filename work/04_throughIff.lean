...
To prove an equivalence we need
to prove both that P implies Q and Q implies P.

For P → Q: We can use the And.left elimination rule
to get P, which means P is true. Meanwhile we can use
the And.right elimination rule to get Q, therefore Q is true
when P is true, proving P → Q.

For Q → P: We can use the And.right elimination rule
to get Q, which means Q is true. Meanwhile we can use
the And.left elimination rule to get P, therefore P is true
when Q is true, proving Q → P.
@@@ -/
...
ANSWER:
It's ok because even if you have this definition,
you'll never be able to call it since it requires
a proof of False to be inputted, which will never exist.
You'll never be able to get a "free"
proof of K by calling exFalsoK because you'll never be
able to call it.
...
-- ANSWER
theorem falseAndImpAnything (P Q : Prop) : (False ∧ P) → Q :=
fun h => nomatch h.left
...
-- ANSWER
We can use And.left to get a proof False from False ∧ P.
Since we now have a proof of False, we're in an impossible
situation which lets us conclude that any proposition,
including Q, is true.
@@@ -/
...
theorem falseImpFalse : False → False :=
  fun a: False => a
/- @@@
When we enter this proof, we assume False is true.
Since we assume it to be true, we can just return it and
say that it's true. When False is assumed to be true,
then False is true.
@@@ -/
